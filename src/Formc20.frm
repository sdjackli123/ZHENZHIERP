...
Adodc1.RecordSource = "select * from v_wwkpd where (" + sql1 + ")   order by 日期,单据"
Adodc1.Refresh
Adodc3.RecordSource = "select  round(sum(出库匹数),2) as 出库匹数,round(sum(isnull(出库重量,0)),2) as 出库重量 from v_wwkpd where (" + sql1 + ")  "
Adodc3.Refresh
...
Formc18.Text2 = Adodc1.Recordset.Fields(15)
Unload Me
End If
End Sub
