...
Begin VB.Form Formw129 
   ...
   Begin VB.ComboBox Combo1 
      ...
      EndProperty
      Height          =   405
      ItemData        =   "Formw129.frx":0016
      Left            =   10440
      List            =   "Formw129.frx":002F
      TabIndex        =   1
      Text            =   "Combo1"
      Top             =   720
      Width           =   2055
   ...
   End
End
Attribute VB_Name = "Formw129"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Dim conn As ADODB.Connection: Dim RD As ADODB.Recordset: Dim bh As String
Public c, r As Integer

Private Sub Command1_Click()
'On Error Resume Next
'If Combo1.Text = "" Then
'Adodc9.RecordSource = "select * from xxclf order by 部门,编号"
'Adodc9.Refresh
'Else
'Adodc9.RecordSource = "select * from xxclf where  部门='" & Combo1.Text & "' order by 编号"
'Adodc9.Refresh
'End If
If Combo1.Text = "" Then
Adodc9.RecordSource = "select * from xxcl  order by 部门,编号,工序"
Adodc9.Refresh
Adodc18.RecordSource = "select 部门,Round(sum(产量),2) as 总产量, Round(sum(工资),2) as 总工资 from xxcl where 工序 not like '总计'   GROUP BY  部门 order by 部门 "
Adodc18.Refresh
Else
Adodc9.RecordSource = "select * from xxcl where  部门='" & Combo1.Text & "'  order by 编号,工序"
Adodc9.Refresh
Adodc18.RecordSource = "select 部门,Round(sum(产量),2) as 总产量, Round(sum(工资),2) as 总工资 from xxcl where 部门='" & Combo1.Text & "' and 工序 not like '总计' GROUP BY  部门 order by 部门 "
Adodc18.Refresh
End If
...
Private Sub Command2_Click()
    ...
    t1 = Format(Trim(Text4.Text) & Space(2) & Text2(0) & ":" & Text2(1) & ":" & Text2(2), "yyyy-MM-dd HH:mm:ss")
    t2 = Format(Trim(Text5.Text) & Space(2) & Text8(0) & ":" & Text8(1) & ":" & Text8(2), "yyyy-MM-dd HH:mm:ss")

    If MsgBox("准备日期为" & t1 & ":" & t2 & "吗？", vbYesNo) = vbNo Then Exit Sub
    ...
    g_Cmd.CommandText = "cjgzzbxx('" & t1 & "','" & t2 & "')"        ' 表示调用哪个存储过程
...
Set g_Cmd = New Command
    ...
    g_Cmd.ActiveConnection = g_Con          ' 连接到数据库
    g_Cmd.CommandType = adCmdStoredProc     ' 表示cmd的类型为存储过程
    g_Cmd.CommandText = "cjgzsc"        ' 表示调用哪个存储过程
    g_Cmd.Execute           ' 执行存储过程
g_Cmd.Cancel
...
Private Sub Command6_Click()
 Dim t1 As String
   ...
   t1 = Format(Trim(Text4.Text) & Space(2) & Text2(0) & ":" & Text2(1) & ":" & Text2(2), "yyyy-MM-dd HH:mm:ss")
    t2 = Format(Trim(Text5.Text) & Space(2) & Text8(0) & ":" & Text8(1) & ":" & Text8(2), "yyyy-MM-dd HH:mm:ss")
Call gzdc(VSFlexGrid3, 14, Combo1.Text & "产量" & t1 & "  ---- " & t2 & " ")
End Sub
...
sql1 = "delete  from xxclf"
RD.Open sql1, conn, adOpenStatic, adLockOptimistic


'''''''''排布

sql1 = "insert into xxclf(编号,工序,产量,工资,部门) select left(编号,3),工序,产量,工资,部门 from xxcl where len(编号)=4"
RD.Open sql1, conn, adOpenStatic, adLockOptimistic

Adodc3.RecordSource = "select 编号,工序,产量,工资,部门 from xxcl where len(编号)>4"
Adodc3.Refresh
If Not Adodc3.Recordset.EOF Then
Adodc3.Recordset.MoveFirst
Do While Not Adodc3.Recordset.EOF
m = Len(Adodc3.Recordset.Fields(0)) / 4

zxs = 0
For i = 0 To Len(Adodc3.Recordset.Fields(0)) / 4 - 1
bh = Mid(Adodc3.Recordset.Fields(0), i * 4 + 1, 3)
Adodc4.RecordSource = "select isnull(核算系数1,'') from works1 where 编号='" & bh & "'"
Adodc4.Refresh
If Not Adodc4.Recordset.EOF Then
zxs = zxs + Adodc4.Recordset.Fields(0)   ''''''''''''''''''总系数
End If
Next


cl = Adodc3.Recordset.Fields(2) / m
gz = Adodc3.Recordset.Fields(3)

For i = 0 To Len(Adodc3.Recordset.Fields(0)) / 4 - 1
bh = Mid(Adodc3.Recordset.Fields(0), i * 4 + 1, 3)
Adodc4.RecordSource = "select isnull(核算系数1,0) from works1 where 编号='" & bh & "'"
Adodc4.Refresh
If Not Adodc4.Recordset.EOF Then
If zxs <> 0 Then
fxs = gz / zxs * Adodc4.Recordset.Fields(0) ''''''''''''''''分系数
End If
Else
fxs = 0
End If

sql1 = "insert into xxclf(编号,工序,产量,工资,部门) VALUES('" & bh & "','" & Adodc3.Recordset.Fields(1) & "','" & cl & "','" & fxs & "','" & Adodc3.Recordset.Fields(4) & "')"
RD.Open sql1, conn, adOpenStatic, adLockOptimistic
Next
...
sql1 = "update xxclf set 工序='染棉' where 工序 like '%染棉%'"
sql2 = "update xxclf set 工序='中样' where 工序 like '%中样%'"
sql3 = "delete from xxcl"
sql4 = "insert into xxcl(编号,姓名,工序,产量,工资,部门) select 编号,姓名,工序,round(sum(产量),2),round(sum(工资),2),部门 from xxclf group by 编号,姓名,工序,部门"
sql5 = "update gzscyg set 姓名=员工"
sql6 = "insert into xxcl(编号,工序,产量,工资,部门) select 编号,'总计',round(sum(产量),2),round(sum(工资),2),部门 from xxcl group by 编号,部门"
RD.Open sql1, conn, adOpenStatic, adLockOptimistic
...
Adodc9.RecordSource = "select 编号,姓名,round(sum(工资),2) as 合计工资 from xxcl where 工序='总计' group by 编号,姓名 order by 编号"
Adodc9.Refresh
End Sub
