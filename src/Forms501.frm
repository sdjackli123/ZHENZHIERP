...
Begin VB.Form Forms501 
   ...
   Begin VB.ComboBox Combo1 
      ...
      EndProperty
      Height          =   435
      ItemData        =   "Forms501.frx":0015
      Left            =   840
      List            =   "Forms501.frx":0022
      TabIndex        =   22
      Text            =   "Combo1"
      Top             =   1200
      Width           =   2535
...
Adodc1.RecordSource = "select * FROM PBCL where 时间=cast('" & DTPicker1.value & "' as datetime) AND 班次='" & Combo1.Text & "' ORDER BY 其它系数 DESC"
Adodc1.Refresh
End Sub
...
Call BBDY(VSFlexGrid1, 12, 17, "排布产量")
End Sub
...
Text1(8).Text = ""
Text1(0).Text = ""
Adodc3.RecordSource = "select max(其它系数) FROM PBCL where 时间=cast('" & DTPicker1.value & "' as datetime)"
Adodc3.Refresh
Text1(14).Text = "1"
...
On Error Resume Next
Adodc6.Refresh
Adodc1.RecordSource = "select * FROM PBCL where 时间=cast('" & DTPicker1.value & "' as datetime) ORDER BY 其它系数 DESC"
Adodc1.Refresh
Adodc3.RecordSource = "select max(其它系数) FROM PBCL where 时间=cast('" & DTPicker1.value & "' as datetime)"
Adodc3.Refresh
Text1(14).Text = "1"
...
Adodc1.RecordSource = "select * FROM PBCL where cast(CONVERT(varchar,时间, 23) as datetime) between cast('" & DTPicker2.value & "' as datetime) and cast('" & DTPicker3.value & "' as datetime) order by 时间"
Adodc1.Refresh
Else
Adodc1.RecordSource = "select * FROM PBCL where  锅号='" & Text1(2).Text & "' order by 时间"
Adodc1.Refresh
End If
...
'    g_Cmd.CommandText = "pbccjc"      ' 表示调用哪个存储过程
...
'    g_Cmd.ActiveConnection = g_Con          ' 连接到数据库
'    g_Cmd.CommandType = adCmdStoredProc     ' 表示cmd的类型为存储过程
'    g_Cmd.CommandText = "cjpbtj('" & Text1(0).Text & "','" & Text1(1).Text & "','" & Text1(2).Text & "','" & Text1(3).Text & "','" & Text1(4).Text & "','" & Text1(5).Text & "','" & Text1(6).Text & "','" & Text1(7).Text & "','" & Text1(8).Text & "','" & Text1(9).Text & "','" & Text1(10).Text & "','" & Text1(11).Text & "','" & Text1(12).Text & "','" & Text1(13).Text & "','" & Text1(14).Text & "','" & Text1(15).Text & "','" & Text1(16).Text & "','" & Text1(17).Text & "','" & Text1(18).Text & "')"     ' 表示调用哪个存储过程
'    g_Cmd.Execute           ' 执行存储过程
'    g_Cmd.Cancel
...
Adodc1.ConnectionString = "Provider=SQLOLEDB.1;Password=fjrj;Persist Security Info=True;User ID=sa;Initial Catalog=zzpr;Data Source=192.168.1.254"
Adodc1.RecordSource = "select * FROM PBCL where 时间=cast('" & DTPicker1.value & "' as datetime)"
Adodc1.Refresh
End Sub
...
Text1(10).Text = ""
Else
Adodc11.RecordSource = "select isnull(sum(毛胚重量),1),isnull(sum(实际匹数),0) from mpbh where 锅号='" & Text1(2).Text & "'"
Adodc11.Refresh
...
List1.Clear
Adodc13.RecordSource = "SELECT distinct 锅号 FROM kpd where  锅号 not like '%f%' and 锅号 in(select distinct 锅号 from mpbh where 出库日期=cast('" & DTPicker1.value & "' as datetime) and 配缸负责 like '%'+'" & Text2 & "'+'%') and 锅号 not in(select distinct 锅号 from pbcl where  工艺编号 like '%'+'" & Text1(11) & "'+'%')"
Adodc13.Refresh
...
Case 2
Adodc6.ConnectionString = "Provider=SQLOLEDB.1;Password=fjrj;Persist Security Info=True;User ID=sa;Initial Catalog=zzpr;Data Source=192.168.1.254"
Adodc6.RecordSource = "select 单号,客户名称,品名,色别+色名 as 色别,重量 from kpd where 锅号='" & Text1(2).Text & "' and 锅号 not like '%f%'"
Adodc6.Refresh
If Adodc6.Recordset.EOF Then
Text1(10).Text = ""
