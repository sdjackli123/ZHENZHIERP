...
Begin VB.Form Forms502 
   ...
   Begin VB.ComboBox Combo2 
      ...
      EndProperty
      Height          =   360
      ItemData        =   "Forms502.frx":0000
      Left            =   840
      List            =   "Forms502.frx":001C
      TabIndex        =   55
      Text            =   "Combo1"
      Top             =   3840
      Width           =   2535
   ...
   Begin VB.ComboBox Combo1 
      ...
      EndProperty
      Height          =   435
      ItemData        =   "Forms502.frx":0055
      Left            =   840
      List            =   "Forms502.frx":0062
      TabIndex        =   3
      Text            =   "Combo1"
      Top             =   1320
      Width           =   2535
...
Adodc1.RecordSource = "select * FROM XDCL where 时间=cast('" & DTPicker1.value & "' as datetime) AND 班次='" & Combo1.Text & "' ORDER BY 其它系数 DESC"
Adodc1.Refresh
End Sub
...
'    g_Cmd.CommandText = "ytccjc"      ' 表示调用哪个存储过程
...
'    g_Cmd.ActiveConnection = g_Con          ' 连接到数据库
'    g_Cmd.CommandType = adCmdStoredProc     ' 表示cmd的类型为存储过程
'    g_Cmd.CommandText = "cjxdtj('" & Text1(0).Text & "','" & Text1(1).Text & "','" & Text1(2).Text & "','" & Text1(3).Text & "','" & Text1(4).Text & "','" & Text1(5).Text & "','" & Text1(6).Text & "','" & Text1(7).Text & "','" & Text1(8).Text & "','" & Text1(9).Text & "','" & Text1(10).Text & "','" & Text1(11).Text & "','" & Text1(12).Text & "','" & Text1(13).Text & "','" & Text1(14).Text & "','" & Text1(15).Text & "','" & Text1(16).Text & "','" & Text1(17).Text & "','" & Text1(18).Text & "')"     ' 表示调用哪个存储过程
'    g_Cmd.Execute           ' 执行存储过程
'    g_Cmd.Cancel
'MsgBox ("超出产量")
'Else
...
Text1(0).Text = ""
Adodc3.RecordSource = "select max(其它系数) FROM XDCL where 时间=cast('" & DTPicker1.value & "' as datetime)"
Adodc3.Refresh
Text1(14).Text = "1"
...
Call BBDY(VSFlexGrid1, 11, 16, "小定型产量")
End Sub
...
On Error Resume Next
Adodc6.Refresh
Adodc1.RecordSource = "select * FROM XDCL where 时间=cast('" & DTPicker1.value & "' as datetime) ORDER BY 其它系数 DESC"
Adodc1.Refresh
Adodc3.RecordSource = "select max(其它系数) FROM XDCL where 时间=cast('" & DTPicker1.value & "' as datetime)"
Adodc3.Refresh
Text1(14).Text = "1"
...
Adodc1.RecordSource = "select * FROM XDCL where cast(CONVERT(varchar,时间, 23) as datetime) between cast('" & DTPicker2.value & "' as datetime) and cast('" & DTPicker3.value & "' as datetime) order by 时间"
Adodc1.Refresh
Else
Adodc1.RecordSource = "select * FROM XDCL where  锅号='" & Text1(2).Text & "' order by 时间"
Adodc1.Refresh
End If
...
Adodc1.ConnectionString = "Provider=SQLOLEDB.1;Password=fjrj;Persist Security Info=True;User ID=sa;Initial Catalog=zzpr;Data Source=192.168.1.254"
Adodc1.RecordSource = "select * FROM XDCL where 时间=cast('" & DTPicker1.value & "' as datetime)"
Adodc1.Refresh
End Sub
...
Adodc5.RecordSource = "SELECT 工序其它系数 FROM gyshd WHERE 工艺编号 between '8001' and '9000' GROUP BY 工序其它系数"
Adodc5.Refresh
