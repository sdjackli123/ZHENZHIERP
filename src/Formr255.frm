...
Begin VB.Form Formr255 
   ...
   Begin VSFlex8UCtl.VSFlexGrid VSFlexGrid1 
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"Formr255.frx":4434
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   Begin VB.ComboBox Combo1 
      ...
      EndProperty
      Height          =   360
      ItemData        =   "Formr255.frx":4533
      Left            =   4440
      List            =   "Formr255.frx":453D
      TabIndex        =   50
      Text            =   "Combo1"
      Top             =   3480
      Width           =   1335
...
Adodc7.RecordSource = "select * from rhzh where 染料名称='" & Text1(1).text & "'"
Adodc7.Refresh
If Adodc7.Recordset.EOF Then
X = InputBox$("请输入提示数量：", Text1(1).text, "10")
Y = InputBox$("请输入提示标志：", Text1(1).text, "10")
KL = 1
End If
...
Adodc7.Recordset.Fields(0) = Text1(1).text
Adodc7.Recordset.Fields(1) = Text1(2).text
Adodc7.Recordset.Fields(2) = X
Adodc7.Recordset.Fields(3) = Adodc7.Recordset.RecordCount + 1
Adodc7.Recordset.Fields(4) = Y
Adodc7.Recordset.Fields(5) = DataCombo2.text
Adodc7.Recordset.Update
Else

Adodc7.Recordset.Fields(1) = Val(Text1(2).text) + Adodc7.Recordset.Fields(1)
Adodc7.Recordset.Update
End If
Adodc7.Refresh
...
Adodc2.RecordSource = "select round(sum(合计金额),2) as 合计 from mx where 单据号='" & Text1(11) & "'"
Adodc2.Refresh
...
Adodc4.Refresh
Adodc3.Refresh
Adodc1.RecordSource = "select *  from mx where  单据号='" & Text1(11).text & "'"
Adodc1.Refresh
Text1(7).text = Adodc1.Recordset.RecordCount + 1
Adodc2.RecordSource = "select round(sum(合计金额),2) as 合计 from mx where 单据号='" & Text1(11) & "'"
Adodc2.Refresh
...
Adodc6.RecordSource = "SELECT * FROM rlrkdj where 单据编号='" & yhdm & "'"
Adodc6.Refresh
...
Call rhlrk(Adodc1, Text1(11).text)
End Sub

Private Sub Command14_Click()
If Text1(1).text = "" Then
Adodc1.RecordSource = "select 供应单位,染化助库名,名称,入库数量,单价,合计金额,单据号,入库时间  from mx where 库别='" & Combo1.text & "' AND 入库时间 between  '" & Text2.text & "' and   '" & Text3.text & "'order by 入库时间"
Adodc1.Refresh
Else
Adodc1.RecordSource = "select 供应单位,染化助库名,名称,入库数量,单价,合计金额,单据号,入库时间  from mx where 库别='" & Combo1.text & "' AND 入库时间 between  '" & Text2.text & "' and   '" & Text3.text & "'AND 名称='" & Text1(1).text & "' order by 入库时间"
Adodc1.Refresh
End If
End Sub
...
On Error Resume Next
If Text1(5).text = "" Then
Adodc1.RecordSource = "select 供应单位,染化助库名,名称,入库数量,单价,合计金额,单据号,入库时间  from mx where 库别='" & Combo1.text & "' AND 入库时间 between  '" & Text2.text & "' and   '" & Text3.text & "'order by 入库时间,单据号"
Adodc1.Refresh
Else
Adodc1.RecordSource = "select 供应单位,染化助库名,名称,入库数量,单价,合计金额,单据号,入库时间 from mx where 库别='" & Combo1.text & "' AND 入库时间 between  '" & Text2.text & "' and   '" & Text3.text & "'AND 供应单位='" & Text1(5).text & "' order by 入库时间,单据号"
Adodc1.Refresh
End If
End Sub
...
Call OutadodcToExcel(VSFlexGrid1, 6, "入库明细")
End Sub
...
Adodc1.RecordSource = "select * from MX WHERE 库别='采购入库' order by 入库时间,VAL(IP)"
Adodc1.Refresh
End Sub
...
Adodc2.ConnectionString = "Provider=SQLOLEDB.1;Password=fjrj;Persist Security Info=True;User ID=sa;Initial Catalog=zzpr;Data Source=tyfw"
