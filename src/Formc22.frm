...
sql1 = sql1 + "备注 like '%'+'" & DataCombo4.Text & "'+'%' and "
End If
...
Adodc1.RecordSource = "select * from v_bmd_gpwt where (" + sql1 + ")  order by 日期,锅号,匹号"
Adodc1.Refresh
Adodc3.RecordSource = "select  count(匹号) as 匹数,round(sum(isnull(光胚重量,0)),2) as 重量 from v_bmd_gpwt where (" + sql1 + ") "
Adodc3.Refresh
