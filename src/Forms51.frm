...
Private Sub Command1_Click() ''''''录入打印
...
Adodc1.RecordSource = "select * from bmd where  锅号='" & DataCombo4(1).Text & "' and 缸号='" & DataCombo4(15).Text & "' and 定型='大定型' order by 匹号 desc"
Adodc1.Refresh
...
Private Sub Command5_Click() '''标签打印
On Error Resume Next
'fs = Val(Text7.Text)    '''''打印份数
'If fs <= 0 Then
        'MsgBox "打印份数必须大于0"
