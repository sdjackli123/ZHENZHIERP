...
Adodc10.RecordSource = "select distinct 单据 from wwkpd where 锅号='" & DataCombo1(1) & "' and 品名='" & DataCombo1(2) & "'"
Adodc10.Refresh
If Not Adodc10.Recordset.EOF Then
If MsgBox("已经出库单据是  " + Adodc10.Recordset.Fields(0) + "确定重复出库吗？", vbYesNo) = vbNo Then Exit Sub
End If
...
Set g_Cmd = New Command
    ...
    g_Cmd.CommandText = "wwkpdlr('" & DataCombo1(0).Text & "','" & DataCombo1(1).Text & "','" & DataCombo1(2).Text & "','" & DataCombo1(3).Text & "','" & DataCombo1(4).Text & "','" & DataCombo1(5).Text & "','" & DataCombo1(6).Text & "','" & DataCombo1(7).Text & "','" & DataCombo1(8) & "','" & DataCombo1(9) & "','" & DTPicker1.value & "','" & Text1 & "','" & DataCombo2.Text & "','" & Text2 & "','" & Text3 & "','" & Text4 & "')"        ' 表示调用哪个存储过程
    g_Cmd.Execute           ' 执行存储过程
g_Cmd.Cancel
Adodc1.RecordSource = "SELECT * FROM wwkpd where 单据='" & Text2 & "' order by 序号"
Adodc1.Refresh

Adodc5.RecordSource = "SELECT 序号 FROM wwkpd WHERE 单据='" & Text2 & "' ORDER BY 序号 DESC"
Adodc5.Refresh
...
Adodc1.Recordset.Fields(15) = Text4
...
Adodc3.RecordSource = "SELECT * FROM wwkpd where 日期=cast('" & DTPicker1.value & "' as datetime)"
Adodc3.Refresh

If Adodc3.Recordset.EOF Then
Text2 = Format(CDate(DTPicker1.value), "yymmdd") + "001"
Else
Adodc3.RecordSource = "SELECT max(right(单据,len(单据)-6)) FROM wwkpd where 日期=cast('" & DTPicker1.value & "' as datetime)"
Adodc3.Refresh
...
Adodc9.RecordSource = "select isnull(count(序号),0) from wwkpd where 单据='" & Text2 & "'"
Adodc9.Refresh
If Not Adodc9.Recordset.EOF Then
hs = Adodc9.Recordset.Fields(0)
If hs > 0 Then
...
ZS = Int(hs / 5) + 1
End If
Adodc9.RecordSource = "select isnull(sum(匹数),0),isnull(sum(重量),0) from wwkpd where 单据='" & Text2 & "'"
Adodc9.Refresh
ps = Adodc9.Recordset.Fields(0)
fhsl = Format(Adodc9.Recordset.Fields(1), "#0.00")
For i = 0 To ZS - 1
Call DXDY(Adodc11, Adodc12, Text2, i * 5 + 1, i * 5 + 5, ps, fhsl)
Next
End If
End If
End Sub
...
wwdm = 1
Formc20.DataCombo1 = DataCombo1(0)
Formc20.Show
End Sub
...
Adodc2.RecordSource = "SELECT  客户名称,锅号,品名,技术要求 as 克重,光胚幅宽,色别,匹数,重量,日期,类别 FROM kPD where 锅号= '" & DataCombo1(1).Text & "  ' order by  ip "
Adodc2.Refresh
       Case 9
Adodc1.ConnectionString = "Provider=SQLOLEDB.1;Password=fjrj;Persist Security Info=True;User ID=sa;Initial Catalog=zzpr;Data Source=192.168.1.254"
Adodc1.RecordSource = "SELECT * FROM wwkpd where 单据='" & Text2 & "' order by 序号"
...
Text4 = Adodc1.Recordset.Fields(15)
End Sub
